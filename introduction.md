...
**z1_controller** stores the code to control the z1 robot directly.

**z1_sdk**: contains the interfaces used for z1 robot control.

**unitree_ros**: contains the unitree products, includes Go1, A1, Aliengo , Laikago and z1 simulation files.
...
Use a cable to connect the robot (which needs to connect to the right backup network port) and PC, then execute python3 unitreeArmTools.py and enter command as prompted.
...
This folder contains the armModel class, which contais functions such as forward and inverse kinematics, inverse dynamics, and spatial Jacobial calculations of the robot.
...
2. armCtrlInJointCtrl()

The function is equivalen to further encapsulation on the basis of *highcmd_development*. The user is only to control the direction of joint rotation instead of entering joint command
...
In cartesian space, the user was originally required to control the spatial velocity , that is, unitreeArm.twist to control the robot, and the function was encapsulated on this basis. Then the user can directly control the direction in which the end of the robot was desired.
...
If the user wants to control the

parameters of the motor directly, they can view this example.
